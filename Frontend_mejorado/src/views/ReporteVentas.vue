            ...
            <tbody>
              <template v-for="venta in ventas" :key="venta.id">
                <tr v-for="(detalle, i) in venta.detalles" :key="`${venta.id}-${i}`">
                  <td>{{ venta.id }}</td>
                  <td>{{ detalle.producto.nombre || detalle.producto.tipo }}</td>
                  <td>{{ venta.cliente_nombre || 'N/A' }}</td>
                  <td>{{ detalle.cantidad }}</td>
                  <td>Q{{ toMoney(detalle.precio_unitario) }}</td>
                  <td>Q{{ toMoney(detalle.subtotal) }}</td>
...
const formatearFecha = (fecha) => {
  if (!fecha) return '';
  return new Date(fecha.split('T')[0]).toLocaleDateString('es-GT', { timeZone: 'UTC' });
};
...
// Lógica de Renderizado de Gráficas
const renderEvolucionChart = (data) => {
  ...
  const primaryColorTransparent = 'rgba(20, 31, 61, 0.4)';

  evolucionChartInstance = new Chart(ctx, {
    type: 'line', data: {
      labels: data.map((item) => formatearFecha(item.dia)),
      datasets: [{
        label: 'Total de Ventas por Día', data: data.map((item) => item.total),
        borderColor: primaryColor, backgroundColor: primaryColorTransparent, tension: 0.1, fill: true
      }]
    }, options: {
...
const renderProductosChart = (data) => {
  ...
  productosChartInstance = new Chart(ctx, {
    type: 'bar', data: {
      ...
      datasets: [{
        label: 'Cantidad Vendida', data: data.map((item) => item.total_vendido),
        backgroundColor: [
          getCssVar('--color-action-primary'), getCssVar('--color-action-secondary'), getCssVar('--color-icon-edit'),
          getCssVar('--navbar-background'), getCssVar('--color-action-danger'),
        ]
      }]
    }, options: {
  ...
  metodosPagoChartInstance = new Chart(ctx, {
    type: 'doughnut', data: {
      ...
      datasets: [{
        ...
        backgroundColor: [
          getCssVar('--color-action-primary'), getCssVar('--color-action-secondary'),
          getCssVar('--color-icon-edit'), getCssVar('--color-action-danger'),
        ]
      }]
    }, options: {
      responsive: true, maintainAspectRatio: false,
...
.chart-container-small .chart-placeholder {
  height: 320px;
}
</style>