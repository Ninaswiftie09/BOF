...
import { apiFetch } from '../utils/api'   // o '@/utils/api' si prefieres alias
...
.contabilidad-container{
  ...
  /* SIN padding-top: ya no hay header fijo */
}

/* Resumen */
.summary-section{ display:flex; flex-wrap:wrap; gap:20px; margin-bottom:30px; }
.summary-card{
  flex:1 1 200px; background: var(--color-primary);
  border-radius:8px; padding:20px; text-align:center; color: var(--colo-texto-blanco);
  transition: background-color .3s;
}
.summary-card:hover{ background: var(--color-senary); }
...
/* Operaciones */
.operations-section{
  background: #1e293b; border-radius:8px; padding:20px; color: var(--colo-texto-blanco);
}
.section-title{ margin-bottom:20px; text-transform:uppercase; color:#fff; text-align:center; }

.btn-add{
  background:#1e293b; color:#fff; border:2px solid #fff;
  padding:10px 20px; border-radius:6px; font-weight:bold; cursor:pointer; transition: background-color .3s;
}
.btn-add:hover{ background:#334155; }

.operation-form{
  background:#f9fafb; padding:20px; border-radius:10px; color:#1e293b;
  box-shadow:0 0 10px rgba(0,0,0,.05);
}
.operation-form h3{ margin-bottom:16px; text-align:center; color:#1e293b; }
.form-group{ margin-bottom:15px; }
.form-group label{ display:block; font-weight:bold; margin-bottom:6px; color:#1e293b; }
.form-group select, .form-group input, .form-group textarea{
  width:100%; padding:10px; border-radius:6px; border:1px solid #cbd5e1; font-size:1rem; font-family:inherit;
}

.btn-submit{
  background:#1e293b; color:#fff; border:none; padding:12px 24px;
  border-radius:6px; font-weight:bold; cursor:pointer; text-transform:uppercase;
  transition: background-color .3s; margin-top:10px; width:100%;
}
...
.operations-table th{
  background: var(--color-primary); color:#fff; text-transform:uppercase;
}
.operations-table tr:nth-child(even){ background: var(--color-quaternary); color:#003366; }
.operations-table tr:nth-child(odd){ background:#1e293b; color:#fff; }
.operations-table tr:hover{ background:#2B5CA8; color:#fff; }

.btn-view{
  background: var(--color-primary); color:#fff; border:none; padding:8px 16px; border-radius:4px; cursor:pointer;
  text-transform:uppercase; transition: background-color .3s;
}
.btn-view:hover{ background: var(--color-senary); }
</style>
